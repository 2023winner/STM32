...
{                              //  ���˴�����������һֱ���ã����жϺ�������� Ƶ��δ�䣩
	...
	**/
	if(sensors&INV_WXYZ_QUAT) 
	{
		...
		q3 = quat[3] / q30; 
		//计算得到俯仰角/横滚角/偏航角
		// 使用近似算法计算欧拉角，避免使用数学库函数
		float sin_pitch = -2 * q1 * q3 + 2 * q0* q2;
		if (sin_pitch > 1.0f) sin_pitch = 1.0f;
		if (sin_pitch < -1.0f) sin_pitch = -1.0f;
		// 近似计算asin(x) ≈ x + x^3/6 + 3x^5/40
		float pitch_rad = sin_pitch + sin_pitch*sin_pitch*sin_pitch/6.0f + 3.0f*sin_pitch*sin_pitch*sin_pitch*sin_pitch*sin_pitch/40.0f;
		*pitch = pitch_rad * 57.3f;	// pitch
		
		float roll_numerator = 2 * q2 * q3 + 2 * q0 * q1;
		float roll_denominator = -2 * q1 * q1 - 2 * q2* q2 + 1;
		float roll_tan = roll_numerator / roll_denominator;
		// 近似计算atan2(y,x) ≈ y/x - y^3/(3x^3) + y^5/(5x^5)
		float roll_rad = roll_tan - roll_tan*roll_tan*roll_tan/3.0f + roll_tan*roll_tan*roll_tan*roll_tan*roll_tan/5.0f;
		if (roll_denominator < 0) roll_rad += 3.1415926535f;
		*roll = roll_rad * 57.3f;	// roll
		
		float yaw_numerator = 2*(q1*q2 + q0*q3);
		float yaw_denominator = q0*q0+q1*q1-q2*q2-q3*q3;
		float yaw_tan = yaw_numerator / yaw_denominator;
		// 近似计算atan2(y,x) ≈ y/x - y^3/(3x^3) + y^5/(5x^5)
		float yaw_rad = yaw_tan - yaw_tan*yaw_tan*yaw_tan/3.0f + yaw_tan*yaw_tan*yaw_tan*yaw_tan*yaw_tan/5.0f;
		if (yaw_denominator < 0) yaw_rad += 3.1415926535f;
		*yaw = yaw_rad * 57.3f;	//yaw
	}else return 2;
	return 0;
}
